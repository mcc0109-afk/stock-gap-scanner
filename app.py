...
# 【緊急修正：網頁設定必須是所有 Streamlit 指令的第一行，以防畫面空白】
...
def find_all_gaps(ticker_symbol, start_date, end_date, gap_type):
    ...
    # 【記憶體極限優化 1】只保留需要的欄位，並強制轉換為記憶體減半的 float32
    stock_data = stock_data[needed_cols].astype('float32')
    
    total_days = len(stock_data)
    
    # 【記憶體極限優化 2】將資料抽出變成純粹的 Numpy 一維陣列 (速度極快，體積極小)
    ...
    # 🔥🔥🔥 關鍵手術：抽出陣列後，直接把最吃記憶體的 Pandas DataFrame 銷毀並回收！
    del stock_data
    gc.collect() 
    
    all_gaps = []
    
    # 【記憶體極限優化 3】放棄使用吃記憶體的 iterrows()，改用高速陣列索引比對
    for i in range(1, total_days):
        gap_date_str = date_vals[i]
        vol_val = float(vol_vals[i])
        
        if gap_type == "下缺口":
            # 判斷是否為下缺口：今天的最高價 < 昨天的最低價
            if high_vals[i] < low_vals[i-1]:
                target_price = float(low_vals[i-1])
                # 尋找未來是否有高點 >= 目標價 (使用 numpy.where 向量運算)
